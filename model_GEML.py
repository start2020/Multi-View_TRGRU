...
def Model(args, mean, std, samples, graph_geo, graph_sem):
    ...
    N = graph_geo.shape[-1]
    R_geo = model_common.multi_gcn(graph_geo, samples, activations=args.GCN_activations, units=args.GCN_units, Ks=args.Ks) #(B,T,N,128)
    print("R_geo ",R_geo)
    S_geo = model_common.multi_gcn(graph_sem, samples, activations=args.GCN_activations, units=args.GCN_units, Ks=args.Ks) #(B,T,N,128)
    print("S_geo ",S_geo)
    ...
    H = model_common.multi_lstm(V, args.RNN_units, args.RNN_type)
    print("H ",H)
    H = tf.reshape(H,shape=[-1, N, H.shape[-1]]) #(BN,128) =>(B,N,128)
    H_m = model_common.fc_layer(H, units=128) # (B,N,128)=>(B,N,128)
    M_pred = tf.matmul(H_m, H, transpose_b=True) #(B,N,128)*(B,N,128)=>(B,N,N)
    print("M_pred ",M_pred)
    P_pred = model_common.fc_layer(H, units=1) #(B,N,128)=>(B,N,1)
    Q_pred = model_common.fc_layer(H,units=1) #(B,N,128)=>(B,N,1)
