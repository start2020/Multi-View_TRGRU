...
def model_1(samples_P_after_W_C):
    ...
    # P = libs.model_common.multi_fc(P, activations="relu", units=[N], drop_rate=None, bn=False, dims=None,
    #                                    is_training=True)
    return P
...
#
#     # outputs = libs.model_common.multi_targets(outputs, std, mean, F_out)
#     return outputs
...
#
#     # outputs = libs.model_common.multi_fc(outputs, activations="relu", units=[288], drop_rate=None, bn=False, dims=None,
#     #                                is_training=True)
#
...
#     # outputs = libs.model_common.multi_targets(outputs, std, mean, F_out) #在output后面加一个fc的话效果更差
...
def model_4(samples_P_before_odtR1_C,samples_P_before_odtW1_C):
    ...
    print(proportion)
    proportion = libs.model_common.multi_fc(proportion, activations="relu", units=[N], drop_rate=None, bn=False, dims=None,
                                   is_training=True)
...
def Model(samples_P_before_C, samples_P_after_W_C, samples_P_before_odtR1_C,samples_P_before_odtD1_C,IN, std, mean, F_out):
    ...
    # outputs = libs.model_common.multi_fc(outputs, activations="relu", units=[N], drop_rate=None, bn=False, dims=None,
    #                                is_training=True)
    In_preds = tf.reduce_sum(outputs, axis=-1)  # (M,P,N) #这个没用的,只是为了代码可以运行而已
    Out_preds = tf.reduce_sum(outputs, axis=-2)  # (M,P,N) #这个没用的,只是为了代码可以运行而已
    return [outputs,In_preds,Out_preds]
