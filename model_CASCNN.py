...
def Model(args, std, mean, X, inflow, outflow):
    ...
    output = model_common.conv2d_layer(output, 1, [1, 1], stride=[1, 1], padding='SAME')
    output = tf.squeeze(output,axis=-1)
    F_out = output.shape[-1] #(B,N,N)
    outputs = model_common.multi_targets(output, std, mean, F_out)
    return outputs
...
def channel_wise(X):
    ...
    C = X.shape[3]
    # (B,1,1,1) =>(B,1,1,C)
    X = model_common.multi_fc(X, activations=['relu','sigmoid'], units=[1, C])
    return X
...
def block(X, output_dims, kernel_size):
    ...
    # (10,20,20,1)
    X = model_common.conv2d_layer(X, output_dims, kernel_size, stride=[1, 1],padding='SAME')
    #(B,N,N,F) => (B,1,1,F)
    E = channel_wise(X)
...
def inout_gate(inflow, outflow):
    # (B,N,1,P)=>(B,N,1,1)
    o_in = model_common.conv2d_layer(inflow, 1, [1,1], stride=[1, 1],padding='SAME')
    o_out = model_common.conv2d_layer(outflow, 1, [1,1], stride=[1, 1],padding='SAME')
