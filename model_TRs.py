...
def module(args, data_name, mean, std, X, SE, T, H, d, bn=False, bn_decay=None):
    ...
    # (B,P,N,N)
    X = model_common.x_embedding(X[..., :-2], D, activations=['relu', None])
    ...
    if args.is_TE:
        ...
        is_TE = False
    # (B,P,N,D)=> (B,P,N,2D)
    # query_key_X = model_common.x_SE_TE(X, SE, TE, is_X=True, is_SE=True, is_TE=True)
    query_key_X = model_common.x_SE_TE(X, SE, TE, is_X=True, is_SE=is_SE, is_TE=is_TE)
    # query_key_X = model_common.x_SE_TE(X, SE, TE, is_X=True, is_SE=False, is_TE=True) 20210526 01方案
    # query_key_X = model_common.x_SE_TE(X, SE, TE, is_X=True, is_SE=True, is_TE=False) 20210526 02方案
    # (B,P,N,2D)=> (B,P,N,D)
    query = model_common.multi_fc(query_key_X, activations=['relu'], units=[D])
    key = model_common.multi_fc(query_key_X, activations=['relu'], units=[D])
    # (B,P,N,N) * (B,P,N,D)=> (B,P,N,D)
    value_X = model_common.x_SE_TE(X, SE, TE, is_X=True, is_SE=is_SE, is_TE=is_TE)
    value = model_common.multi_fc(value_X, activations=['relu'], units=[D])
    ...
    if args.is_RNN:
        ...
        X = FC_end(args,X,P,N)
    # 方案一
    # X = model_common.inverse_positive(X, std, mean, N)
    # eixt()
    return X
...
def TCN_end(args, X):
    ...
    y =  model_common.multi_tcn(x, args.K, args.d, activations=args.TCN_activations, units=args.TCN_units)
    return y[:,:,-1,:] #(b,n,n)